...
type Subscription = { id: number; name: string; status: string; expiresAt?: string; quotaGrantedUsd: number; quotaUsedUsd: number; quotaRemainingUsd: number };
...
export default function Dashboard() {
  ...
  return <div class="space-y-8"><div><p class="eyebrow">Overview</p><h1 class="title">Good to see you{data()?.profile.displayName ? `, ${data()?.profile.displayName}` : ""}.</h1><p class="muted">Your account activity and access at a glance.</p></div>
    ...
    <section class="panel"><div class="section-head"><h2>Subscription</h2><A href="/billing" class="text-sm underline">Billing</A></div><Show when={data()?.subscriptions.items.length} fallback={<p class="muted">No active subscription.</p>}><For each={data()?.subscriptions.items.slice(0, 1)}>{sub => <div class="flex flex-wrap items-center justify-between gap-3"><div><p class="font-medium">{sub.name}</p><p class="muted">{sub.status} · {sub.quotaUsedUsd.toFixed(2)} / {sub.quotaGrantedUsd.toFixed(2)} USD used · {sub.quotaRemainingUsd.toFixed(2)} available</p></div><span class="status">{sub.expiresAt ? `Renews ${new Date(sub.expiresAt).toLocaleDateString()}` : "No expiry"}</span></div>}</For></Show></section>
  </div>;
}
