...
The latest source snapshot is Gateway `3da0d33` and Platform `5a4fb36`.
...
paired registration/revocation audits; browser ceremony and User Web controls are
...
| Platform tests | 186/186, exit 0; Release build 0 warnings/0 errors | 69 Grain tests, 52 Host tests, 24 Admin tests, and 41 Provider mock tests; Host coverage includes versioned Unicode normalization, source-owned external classifier request/status/schema/timeout/cancellation behavior, redacted audit metadata, policy revision schema, successful/retryable propagation, concurrent two-worker claim serialization, staged audits, accounting, media, and reconciliation; Admin coverage adds atomic referral reward replay/conflict evidence, audited operational-metric summaries/alerts, bounded redacted audit export evidence, encrypted proxy/TLS profile validation, bounded active-account monitor checks, and Passkey challenge/credential/counter lifecycle evidence; Provider HTTP tests cover the classifier contract and deterministic fixtures |
| Passkey persistence and ceremony state | Platform `5a4fb36`; temporary PostgreSQL, product migrations 001-031 applied then skipped, targeted Admin and schema tests 1/1 each passed | `passkey_challenges` enforces flow-scoped five-minute one-shot challenges; `passkey_credentials` stores public material and monotonic signature counters; registration/revocation audit rows are transactionally paired. Fido2 registration/authentication endpoints issue normal sessions without private key storage, and the generic greenfield schema test requires every Passkey column. Browser WebAuthn ceremony, User Web controls, anti-enumeration, and distributed abuse limits remain open |
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-031 and skipped all 31 on replay; the migrator image copies the complete directory and includes migration 031 | The targeted Passkey schema gate applies/skips 32 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `5a4fb36`; no source database, CDC, compatibility table, snapshot, or old key was used |
