...
| Platform tests | 90/90, exit 0 | 57 Grain tests, 23 PostgreSQL-connected Host tests, 4 Admin tests, and 6 Provider mock selector tests; includes 20 concurrent ordered accounting credits, replay/conflict detection, SQL hold oversubscription and protected debit, versioned Grain projection, configuration-without-balance replacement, provider capability policy, rolling quota, response replay, price snapshots, expiry recovery, and fault selection |
...
| PostgreSQL migrator | Repository gate started a brand-new PostgreSQL volume, applied current image `a463940c17c2` migrations 000-018, then observed all nineteen as `skip` on an explicit second run, exit 0 | No source database, CDC, compatibility table, snapshot, or old key used |
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `ea83e5a` and Gateway `dc69269` in unique Podman project `scalaapi-smoke-accounting4`; source-built images were Platform `6ef3907de695`, Admin API `adb846e21042`, Gateway `e04cd60778c2`, Provider mock `dcc0f854db07`, migrator `a463940c17c2`, and Admin Web `23769c75ead8`. Product APIs created a zero-balance user, applied/replayed one audited adjustment, rejected conflict/overdraft, configured key/groups/prices, settled/replayed Chat, replaced Platform and Gateway independently, passed isolated 429/500/malformed/disconnect/timeout no-charge assertions, authenticated Garnet, and downloaded the 67-byte MinIO object | Source-owned gate proves ordered accounts, holds, ledger and projection drain from empty volumes. A first exact-source run correctly failed because disconnect can terminate as 502 or 503 depending transport-versus-scheduler timing; `ea83e5a` declares both outcomes while preserving exact monetary assertions. Hosted CI, normalized error contracts, client cancellation, SSE faults, and exact-boundary crash injection remain |
...
| Billable settlement smoke | JSON and SSE completed; SQL-authoritative hold committed; usage outbox processed; one versioned NUMERIC ledger debit per successful lease; account balance equalled ledger sum, max version equalled account version, versions were contiguous/distinct, and projection backlog reached 0. Clean requests after independent Platform/Gateway replacement settled once, while five non-stream fault scenarios left all leases aborted, holds released, and zero usage/log/debit records | Precise dispatch/report/commit crash injection, client cancellation, partial SSE, unknown-charge classification, and periodic reconciliation remain |
| Ordered accounting authority | Administrative adjustments, payment credits/refunds, redeem bonuses, and usage debits use one per-user serialized store. A real-database test launched 20 concurrent effects and observed versions 1-20, exact replay, changed-effect conflict, hold oversubscription rejection, protected debit, final account/ledger equality, and latest projection outbox state. Grain tests prove stale snapshots cannot regress a balance | Add periodic account-ledger-hold-projection reconciliation, operator-visible incidents, and deterministic crash hooks; subscription/affiliate monetary effects adopt the same contract when implemented |
...
4. Normalize the direct-reset 502 versus post-cooldown 503 outcome into one public
