...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web `6bc411b`, and User Web `44d2096`.
...
| Platform tests | 198/198, exit 0; Release build 0 warnings/0 errors | 69 Grain tests, 57 Host tests, 30 Admin tests, and 42 Provider mock tests; Host coverage includes versioned Unicode normalization, source-owned external classifier request/status/schema/timeout/cancellation behavior, redacted audit metadata, policy revision schema, successful/retryable propagation, concurrent two-worker claim serialization, staged audits, accounting, media lifecycle, metadata-only object reconciliation and recovery, reconciliation, serialized subscription quota reservation/settlement, deterministic provider pricing parsing, bounded provider responses, and real PostgreSQL pricing history/replay; Admin coverage adds atomic referral reward replay/conflict evidence, audited operational-metric summaries/alerts, bounded redacted audit export evidence, encrypted proxy/TLS profile validation, bounded active-account monitor checks, Passkey challenge/credential/counter lifecycle evidence, maintenance export redaction/cleanup/replay/conflict evidence, announcement read-state duplicate replay/audit evidence, encrypted email outbox delivery/supersession/retry evidence, and concurrent subscription renewal/expiry/past_due lifecycle evidence; Provider HTTP tests cover classifier and pricing catalog contracts plus deterministic fixtures |
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-037 and skipped all 37 on replay; the migrator image copies the complete directory and includes migration 037 | The targeted empty-schema gate applies/skips 38 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `44d2096`; no source database, CDC, compatibility table, snapshot, or old key was used |
