...
| Platform tests | 89/89, exit 0 | 58 Grain tests, 22 PostgreSQL-connected Host tests, 3 Admin webhook verifier tests, and 6 Provider mock selector tests; includes administrative balance replacement, provider capability policy, deterministic rolling quota precedence/reset, usage-triggered auth invalidation, response replay, immutable price snapshots, expiry recovery, restart-safe outbox claims, idempotent balance-effect replay, and protocol-reachable fault selection |
...
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `cea9519` content and Gateway `dc69269` in unique Podman project `scalaapi-smoke-fault1`; source-built images included Platform `3a21cb70`, Gateway `c07d78f0`, Provider mock `a1b02e15`, migrator `36bed78a`, and Admin Web `c9b504b5`. Product APIs configured user/key/groups/prices, Chat settlement and exact replay passed, Platform and Gateway were independently replaced and each accepted one new billable request, isolated 429/500/malformed-usage/upstream-disconnect/timeout billing assertions passed, authenticated Garnet returned PONG, and MinIO bootstrap plus 67-byte signed download passed | Source-owned Docker/Podman gate is reproducible, catches false restarts and zero-charge failure regressions, and cleans only its project; hosted cross-private-repository CI, client cancellation, SSE faults, and exact-boundary crash injection remain |
