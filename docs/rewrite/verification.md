...
The latest source snapshot is Gateway `6243b2d` and Platform `d126ea5`.
...
| Gateway build and CTest | Clean local build; 109/109, exit 0 | Adds fail-closed OpenAI model-list duplicate/metadata checks, Gemini model name/method/token-limit checks, and Anthropic bounded positive token-count checks to the Embeddings request/response, nested Anthropic usage, SSE, timeout, cancellation, Garnet, retry, and routing coverage |
