...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web/User Web `bcf80e7`.
...
the isolated project; `podman ps -a` was empty afterward. Certificate rotation,
expiry, and partitioned multi-process convergence remain release gates.
...
| Current Platform tests | 243/243, exit 0; Release build 0 warnings/0 errors | 69 Grain, 80 Host, 46 Admin, and 48 Provider tests; Host adds an in-process TLS RESP listener covering configured PEM CA trust, authenticated Garnet traffic, server-name rejection, and the TLS/CA configuration guard; existing coverage includes the PostgreSQL-backed administrative-price precedence regression test, monotonic Garnet revision publication, fixed-label OpenAI classifier metrics/secret-redaction assertions, migrations 044-046 cross-instance snapshot/p95/error-ratio/budget-alert/window-recovery/hosted-worker replay, and backup schema coverage |
...
3. Extend the now-passing Garnet TLS deployment gate with certificate rotation and
   expiry/failure recovery, cache flush and stale-version recovery while TLS is
   enabled, and concurrent Gateway/Platform clients across outage/restart and
   partition scenarios. No Redis process, package, image, CLI, or embedded fallback
