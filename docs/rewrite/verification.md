...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web/User Web `0dd49cf`.
...
| Admin Web | Platform `0dd49cf`; typecheck, production build, and Playwright Chromium smoke passed (`2/2`) | Provider account form supports static headers and OAuth refresh metadata/replacement while list/details expose health but not stored secrets; reconciliation filters incidents, runs manual checks, and submits evidence-backed settle/release commands with a stable key per selected command and replay-safe responses. Content Policy manages rules and exposes propagation changes and alert filters. Operations renders bounded metric summaries and policy-alert evidence with kind/severity filters and explicit refresh. The browser smoke intercepts the Content Policy and Operations contracts, verifies route navigation and tab rendering; live authorization/API and broader operations workflows remain |
...
| Operational metrics and policy alerts | Platform `9848427` replaces raw metric inserts with an authenticated bounded command. Platform `0dd49cf` adds the authenticated Admin Web Operations route with typed metric summaries, kind/severity alert filters, and explicit refresh. The real `OpsMetricsStoreTests` empty-volume run passed 1/1, and the Admin Web Chromium suite passed 2/2 with summary, alert, filter, and refresh coverage | Collector rules, cross-service correlation, alert delivery/recovery, and live authorization/browser workflows remain |
