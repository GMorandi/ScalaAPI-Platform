...
| Current Platform tests | 250/250, exit 0; Release build 0 warnings/0 errors | 69 Grain, 82 Host, 46 Admin, and 53 Provider tests; current additions cover Responses compact upstream path resolution and Provider JSON/SSE/error contracts. The preceding coverage includes stable source-text Responses input-item replay/deletion, idempotent cancellation, stored cancelled retrieval, unknown-ID rejection, PostgreSQL-backed administrative-price precedence, monotonic Garnet revision publication, fixed-label OpenAI classifier metrics/secret-redaction, migrations 044-046 cross-instance snapshot/p95/error-ratio/budget-alert/window-recovery/hosted-worker replay, and backup schema coverage |
