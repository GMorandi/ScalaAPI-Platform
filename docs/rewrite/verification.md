...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web/User Web `0acdf9c`.
...
| Current Platform tests | 240/240, exit 0; Release build 0 warnings/0 errors | 69 Grain, 77 Host, 46 Admin, and 48 Provider tests; Admin adds PostgreSQL-backed backup/restore guardrails; Host includes the PostgreSQL-backed administrative-price precedence regression test, monotonic Garnet revision publication coverage, fixed-label OpenAI classifier metrics/secret-redaction assertions, and migrations 044-046 cross-instance snapshot/p95/error-ratio/budget-alert/window-recovery/hosted-worker replay plus backup schema coverage |
