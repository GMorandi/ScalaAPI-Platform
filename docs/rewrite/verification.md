...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web `de53df4`, and User Web `de53df4`.
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-038 and skipped all 38 on replay; the migrator image copies the complete directory and includes migration 038 | The targeted empty-schema gate applies/skips 39 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `de53df4`; no source database, CDC, compatibility table, snapshot, or old key was used |
...
| Provider payment checkout | Platform `de53df4`; migration 038 was previously applied/skipped on empty PostgreSQL; Admin adapter tests 4/4, Provider Mock HTTP contract 1/1, User Web typecheck/build, and full Release suite 203/203 passed | `/user/payments/create` supports mock and Stripe providers, persists a pending order before the provider call, enforces exact idempotency payload replay/conflict, retries pending orders without a second order, persists a bounded checkout URL, and maps malformed/insecure/oversized/failed provider responses to safe errors. Stripe form requests use Basic secret auth, minor-unit amount, metadata and HTTPS redirect bounds; additional production adapters, browser payment completion, webhook/refund reconciliation, and exact-boundary crash evidence remain |
