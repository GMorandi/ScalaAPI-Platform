...
The latest source snapshot is Gateway `9c7171f` and Platform `b047445`.
...
| User Web | `npm run typecheck` and `npm run build` passed in `user-web` | Independent Solid client covers auth, OAuth callback, dashboard, scoped usage, API keys, billing, and profile; browser tests and recovery UX remain |
...
| User portal contract | User Web uses refresh-aware `/auth`, scoped `/user/usage` and `/user/usage/balance`, API key, payment, subscription, profile, and password contracts; repository test proves usage rows cannot cross user boundaries | Browser automation, password recovery/TOTP setup, redeem/affiliate flows, and real payment provider checkout remain |
