...
The latest source snapshot is Gateway `297b131` and Platform `8368d77`.
Gateway CTest is 102/102. The complete empty-volume gate passed in
...
`disconnect_after_usage` case: valid usage observed before truncated SSE EOF
settled exactly once through the durable outbox. The gate ended with nine
unknown-charge incidents, one audited operator resolution, and eight remaining
open incidents. Temporary smoke containers, volumes, networks, and tags were
removed by the cleanup trap; baseline `apitf_*` development resources remain.
...
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `8368d77` and Gateway `297b131` source snapshots in unique project `scalaapi-platform-dispatch-recovery-0912`; the source-built stack applied all 22 migrations twice, authenticated Garnet, settled/replayed Chat, exercised Platform before-dispatch plus pre/post/pre-ack and Gateway before-dispatch/after-provider-completion crash recovery, replaced both services, ran the Provider matrix, resolved one incident, and persisted/downloaded the MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, ledger/projection drain, exactly-once recovery, safe never-forwarded expiry at both Platform and Gateway boundaries, late usage settlement, actual client cancellation retention, and nine unknown-charge incidents with eight remaining open after one decision. Hosted CI, final-usage replay/reconciliation, the remaining Platform dispatch/worker/Gateway hook matrix, and multi-instance evidence remain |
...
| Billable settlement smoke | JSON and SSE completed; SQL-authoritative hold committed; usage outbox processed; one versioned NUMERIC ledger debit per successful lease; account balance equalled ledger sum, max version equalled account version, versions were contiguous/distinct, and projection backlog reached 0. Platform before-provider-dispatch, pre-settlement, post-settlement, and before-outbox-ack crashes plus Gateway before-provider-dispatch and after-provider-completion crashes were recovered by explicitly starting the same container; never-forwarded work expired safely at both dispatch boundaries, while forwarded ambiguity retained one reconciliation hold. Explicit non-stream and streaming 429/500 attempts released; malformed/disconnect/timeout-before-headers plus streaming disconnect, disconnect-before-output, malformed-usage, invalid-content-type, and downstream client-cancel attempts each retained one unknown hold. The truncated-stream profile settled valid usage exactly once before EOF; Admin settle replay returned `duplicate`; the latest gate ended with eight open incidents after nine were created | Remaining Platform worker/dispatch reclaim details, other Gateway boundaries, and multi-instance recovery remain |
...
| Request idempotency smoke | Concurrent same-key calls produced one 200 and one active-lease 409; after settlement a matching retry returned the original body; different fingerprint produced 409. Platform and Gateway before-provider-dispatch loss safely expired their original held keys; forwarded/unknown outcomes stay blocked in `reconciliation_needed`; late completion and truncated-stream usage finalize the original key once. Platform pre/post/pre-ack and Gateway after-provider-completion crash replays each produced one terminal debit or one retained reconciliation hold | Streaming cancellation/partial output is non-retryable in Gateway source; Platform worker reclaim and multi-instance replay remain |
...
4. Extend the now-normalized `503/provider_unavailable` Provider reset contract to
   ...
   Exercise the remaining dispatch/worker,
   other Gateway, and outbox-acknowledgement boundaries. Require each restart outcome to settle, safely
