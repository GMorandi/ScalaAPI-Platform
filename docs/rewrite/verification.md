...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web `d8788dd`, and User Web `d8788dd`.
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-042 and skipped all 42 on replay; the migrator image copies the complete directory and includes migration 042 | The targeted empty-schema gate applies/skips 43 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `d8788dd`; no source database, CDC, compatibility table, snapshot, or old key was used |
