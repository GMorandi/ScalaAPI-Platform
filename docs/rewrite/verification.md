...
The latest source snapshot is Gateway `eb5734f` and Platform `90597b8`.
...
settlement, and eight remaining open incidents. This smoke used a temporary
...
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `90597b8` and Gateway `eb5734f` source snapshots in unique project `scalaapi-platform-dispatch-retry-0914`; the source-built stack applied all 22 migrations twice, authenticated Garnet, settled/replayed Chat, exercised Platform dispatch retry and active-lease recovery, Platform before-dispatch/after-outbox-claim/pre/post/pre-ack and Gateway before-dispatch/after-provider-completion crash recovery, replaced both services, ran the Provider matrix, resolved one incident, and persisted/downloaded the MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, exactly-once recovery including dispatch retry and outbox claim reclaim, safe never-forwarded expiry at both dispatch boundaries, late usage settlement, actual client cancellation retention, and nine unknown-charge incidents with eight remaining open after one decision. This run used a temporary runtime image assembled from the verified local Gateway build because the pinned Photon commit is unavailable; hosted CI, other Gateway/realtime retry paths, and multi-instance evidence remain |
...
4. Extend the now-normalized `503/provider_unavailable` Provider reset contract to
   ...
   Platform dispatch retry and active-lease recovery are now proven for the
   regular Chat path. Exercise the remaining realtime/other Gateway retry and
