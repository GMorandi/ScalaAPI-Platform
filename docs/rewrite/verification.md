...
The latest source snapshot is Gateway `3da0d33`, Platform and User Web `857ef3b`.
...
The current authentication notification slice adds product migration 034 and a real
empty-schema PostgreSQL check. The first migrator run applied 35 total records
(Orleans plus 001-034), the replay skipped all 35, and `EmailDeliveryTests` passed
...
| Platform tests | 191/191, exit 0; Release build 0 warnings/0 errors | 69 Grain tests, 52 Host tests, 29 Admin tests, and 41 Provider mock tests; Host coverage includes versioned Unicode normalization, source-owned external classifier request/status/schema/timeout/cancellation behavior, redacted audit metadata, policy revision schema, successful/retryable propagation, concurrent two-worker claim serialization, staged audits, accounting, media, and reconciliation; Admin coverage adds atomic referral reward replay/conflict evidence, audited operational-metric summaries/alerts, bounded redacted audit export evidence, encrypted proxy/TLS profile validation, bounded active-account monitor checks, Passkey challenge/credential/counter lifecycle evidence, maintenance export redaction/cleanup/replay/conflict evidence, announcement read-state duplicate replay/audit evidence, and encrypted email outbox delivery/supersession/retry evidence; Provider HTTP tests cover the classifier contract and deterministic fixtures |
...
| User Web | `npm run typecheck` and `npm run build` passed in `user-web` at `857ef3b` | Independent Solid client covers auth, OAuth callback, password recovery with action-link hydration, email verification with action-link hydration, dashboard announcements/read tracking, scoped usage, API keys, active-plan subscription purchase/cancel/renew, redeem codes, referral summary, profile, TOTP setup/verify/disable, and Passkey registration/revocation/sign-in option conversion; browser tests, real authenticator ceremony, mail receipt, and backup-code sign-in remain |
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-034 and skipped all 34 on replay; the migrator image copies the complete directory and includes migration 034 | The targeted authentication notification schema gate applies/skips 35 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `857ef3b`; no source database, CDC, compatibility table, snapshot, or old key was used |
