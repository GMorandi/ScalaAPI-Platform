...
The latest source snapshot is Gateway `3da0d33`, Platform and User Web `acb1c66`.
...
| Platform tests | 188/188, exit 0; Release build 0 warnings/0 errors | 69 Grain tests, 52 Host tests, 26 Admin tests, and 41 Provider mock tests; Host coverage includes versioned Unicode normalization, source-owned external classifier request/status/schema/timeout/cancellation behavior, redacted audit metadata, policy revision schema, successful/retryable propagation, concurrent two-worker claim serialization, staged audits, accounting, media, and reconciliation; Admin coverage adds atomic referral reward replay/conflict evidence, audited operational-metric summaries/alerts, bounded redacted audit export evidence, encrypted proxy/TLS profile validation, bounded active-account monitor checks, Passkey challenge/credential/counter lifecycle evidence, maintenance export redaction/cleanup/replay/conflict evidence, and announcement read-state duplicate replay/audit evidence; Provider HTTP tests cover the classifier contract and deterministic fixtures |
...
| User Web | `npm run typecheck` and `npm run build` passed in `user-web` at `acb1c66` | Independent Solid client covers auth, OAuth callback, password recovery, email verification, dashboard announcements/read tracking, scoped usage, API keys, active-plan subscription purchase/cancel/renew, redeem codes, referral summary, profile, TOTP setup/verify/disable, and Passkey registration/revocation/sign-in option conversion; browser tests, real authenticator ceremony, and backup-code sign-in remain |
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-033 and skipped all 33 on replay; the migrator image copies the complete directory and includes migration 033 | The targeted announcements schema gate applies/skips 34 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `acb1c66`; no source database, CDC, compatibility table, snapshot, or old key was used |
