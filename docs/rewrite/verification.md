...
The latest source snapshot is Platform/Admin Web/User Web `b5586cf` and Gateway
`418da3a`.

The latest source-built project `scalaapi-media-restart-0810` exited zero.
...
orphan pass, bounded archive downloads, and terminal retention cleanup. Platform
...
protected. Restart restore, per-item reconciliation, and deployment-scale
...
and proves a running operation resumes after Platform replacement. Per-item
reconciliation and full MinIO lifecycle evidence remain open.
...
| Current Platform tests | 263/263, exit 0; Release build 0 warnings/0 errors | 69 Grain, 87 Host, 46 Admin, and 61 Provider tests; current additions cover signed S3 list pagination, media orphan protection/deletion, bounded batch archive ZIP creation and manifest/error entries, retryable terminal media retention cleanup, API-key-isolated durable image batch listing, and Provider image task/batch cancellation contracts, alongside three Embeddings provider profiles, catalog entries, dimensions, token accounting, and fault boundaries. The preceding coverage includes Responses compact path resolution, stable source-text input-item replay/deletion, idempotent cancellation, pricing precedence, Garnet revision publication, classifier metrics/secret-redaction, migrations 044-047, and backup schema coverage |
...
| PostgreSQL migrator | The current baseline is product migrations 001-047 plus the image-owned Orleans baseline (48 records); the `scalaapi-media-retention-0810a` empty PostgreSQL run applied and replay-skipped all 47 product migrations. Migration 043 widens only the greenfield classifier constraints to local/external/openai, 044-045 add cross-instance classifier metric snapshots and durable budget-alert state, 046 adds backup/restore job state, and 047 adds independent media retention deadlines | The current empty-volume bootstrap evidence is authoritative; no source database, CDC, compatibility table, snapshot, or old key was used |
...
| Media lifecycle smoke | Image and video create calls returned durable `med_*` IDs; Platform polling copied provider bytes to MinIO, persisted `object_status=stored`, object key/ETag/size, and returned one-hour SigV4 URLs that downloaded `image/png` or `video/mp4`; the `scalaapi-media-batch-0810a` extension created a batch, listed it through PostgreSQL, projected its public `data` items, and retained stored object metadata; `scalaapi-media-cancel-0810c` additionally called the Provider batch cancel endpoint, retained durable `canceled` state, and counted its conservative unknown-charge hold in reconciliation; `scalaapi-media-download-0810d` persisted a bounded ZIP with manifest/error entries and verified the signed redirect; `scalaapi-media-retention-0810a` forced the retention deadline and verified S3 deletion plus cleared metadata; `scalaapi-media-restart-0810` resumed a running operation after Platform replacement; batch `delete_outputs` remained idempotent; migration 037 metadata-only reconciliation verifies signed HEAD existence/size/ETag and restores a failed metadata row after the object returns; Platform `1d7ec4f` adds signed paginated object listing and a grace-period orphan pass, with Host tests protecting referenced/young objects and deleting aged orphans | Full MinIO restart, per-item download reconciliation, and deployment-scale lifecycle remain |
