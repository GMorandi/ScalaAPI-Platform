...
| Platform tests | 77/77, exit 0 | 55 grain tests plus 22 PostgreSQL-connected Host tests; includes deterministic rolling quota precedence/reset, usage-triggered auth invalidation, response replay, immutable price snapshots, expiry recovery, restart-safe outbox claims, and idempotent balance-effect replay |
...
| PostgreSQL migrator | Current image contains 000-012; repeated invocation skipped all thirteen recorded migrations, exit 0 | No source database, CDC, or compatibility tables used; a truly empty-volume replay remains a release gate |
