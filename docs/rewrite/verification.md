...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web/User Web `e5c341d`.
...
3. Verify the full Garnet TLS deployment: server-side TLS flags and certificate
   mount, Platform/Gateway CA trust and DNS identity, cache flush, stale-version
   recovery, restart, certificate rotation, and concurrent Gateway/Platform
   clients. No Redis process, package, image, CLI, or embedded fallback may appear
   in the stack.
