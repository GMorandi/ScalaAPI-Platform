...
The latest source snapshot is Gateway `297b131` and Platform `a0ea559`.
...
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `a0ea559` and Gateway `297b131` source snapshots in unique project `scalaapi-gateway-recovery-0907`; the source-built stack applied all 22 migrations twice, authenticated Garnet, settled/replayed Chat, exercised Platform pre/post/pre-ack and Gateway after-provider-completion crash recovery, replaced both services, ran the Provider matrix, resolved one incident, and persisted/downloaded the MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, ledger/projection drain, exactly-once recovery, late usage settlement, actual client cancellation retention, and ten unknown-charge incidents with nine remaining open after one decision. Hosted CI, final-usage replay/reconciliation, the remaining dispatch/worker/Gateway hook matrix, and multi-instance evidence remain |
...
| Billable settlement smoke | JSON and SSE completed; SQL-authoritative hold committed; usage outbox processed; one versioned NUMERIC ledger debit per successful lease; account balance equalled ledger sum, max version equalled account version, versions were contiguous/distinct, and projection backlog reached 0. Platform pre-settlement, post-settlement, and before-outbox-ack crashes plus Gateway after-provider-completion crash were recovered by explicitly starting the same container; each replay produced one debit or retained one reconciliation hold. Explicit non-stream and streaming 429/500 attempts released; malformed/disconnect/timeout-before-headers plus streaming disconnect, disconnect-before-output, malformed-usage, invalid-content-type, and downstream client-cancel attempts each retained one unknown hold. The truncated-stream profile settled valid usage exactly once before EOF; Admin settle replay returned `duplicate`; the latest gate ended with nine open incidents after ten were created | Remaining dispatch/worker/Gateway boundaries and multi-instance recovery remain |
...
| Reconciliation incident lifecycle | A real PostgreSQL test deliberately corrupted one account, one terminal hold, and one Grain projection while creating an additional unknown-charge lease. The Admin API settled one incident with actor/reason/evidence, replayed the same command, and the next run preserved its resolved state. The latest full-stack fault gate additionally persisted ten dispatch/Gateway-evidenced incidents, including actual downstream client cancellation, Gateway after-provider-completion process loss, and a late-usage stream that settled outside the incident set; one audited decision left nine open incidents | Add alert delivery, multi-Silo concurrency, and the remaining dispatch/worker/Gateway hook boundaries |
...
4. Extend the now-normalized `503/provider_unavailable` Provider reset contract to
   ...
   recovery are source-smoke proven; Gateway after-provider-completion recovery
   is also covered by the latest smoke. Exercise the remaining dispatch/worker,
