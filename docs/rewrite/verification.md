...
The latest source snapshot is Gateway `9c7171f` and Platform `ea0e7f2`.
...
| PostgreSQL migrator | A fresh PostgreSQL volume applied migrations 000-023, including `022-auth-totp-state.sql` and `023-auth-oauth-states.sql`, then observed all twenty-four as `skip` on an explicit second run, exit 0 | No source database, CDC, compatibility table, snapshot, or old key used |
