...
The latest source snapshot is Gateway `9c7171f` and Platform `1ffe357`.
...
| Platform tests | 124/124, exit 0 | 59 Grain tests, 33 Host tests, 9 Admin tests, and 23 Provider mock tests; adds OAuth Provider credential lease serialization, atomic access/refresh rotation, metadata-update secret retention, refresh failure/backoff state, HTTPS/form token exchange, rotated-token parsing, provider error-body redaction, and real HTTP Provider mock rotation/revocation/malformed/oversized response contracts to the ordered accounting, identity, reconciliation, and user-scoping evidence |
...
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `1ffe357` and Gateway `9c7171f` in unique project `scalaapi-oauth-refresh-20260809`; the clean Gateway image was built with the immutable Photon pin and full FetchContent history. The stack applied all 24 migrations (including the Orleans schema), skipped all 24 on the second run, authenticated Garnet, rotated the expired OAuth seed before Chat dispatch, settled/replayed Chat and realtime WebSocket, exercised restart/recovery and the Provider matrix, resolved one incident, and persisted/downloaded the MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, exactly-once recovery including dispatch retry and outbox claim reclaim, realtime usage settlement, OAuth rotation over the mock HTTP contract, safe never-forwarded expiry, late usage settlement, actual client cancellation retention, and nine unknown-charge incidents with eight remaining open after one decision. Hosted CI, runtime WebSocket soak, multi-instance recovery, and cross-protocol automation remain |
