...
The latest source snapshot is Platform/Admin Web/User Web `5f04bfd` and Gateway
`22f65d4`.
...
| Current Platform tests | 256/256, exit 0; Release build 0 warnings/0 errors | 69 Grain, 82 Host, 46 Admin, and 59 Provider tests; current additions cover three Embeddings provider profiles, catalog entries, dimensions, token accounting, and fault boundaries. The preceding coverage includes Responses compact path resolution, stable source-text input-item replay/deletion, idempotent cancellation, pricing precedence, Garnet revision publication, classifier metrics/secret-redaction, migrations 044-046, and backup schema coverage |
