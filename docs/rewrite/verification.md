...
The latest source snapshot is Gateway `3da0d33`, Platform and User Web `d71fe8b`.
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-036 and skipped all 36 on replay; the migrator image copies the complete directory and includes migration 036 | The targeted empty-schema gate applies/skips 37 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `d71fe8b`; no source database, CDC, compatibility table, snapshot, or old key was used |
