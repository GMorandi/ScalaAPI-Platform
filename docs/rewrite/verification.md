...
The latest source snapshot is Gateway `9c7171f` and Platform `9491ea9`.
...
| Platform tests | 102/102, exit 0 | 57 Grain tests, 30 PostgreSQL-connected Host tests, 9 Admin tests, and 6 Provider mock selector tests; includes ordered accounting, replay/conflict, SQL holds, versioned projection, held/forwarded/output-started transitions, safe held expiry, unknown-abort hold preservation, audited operator settle/release, resolution replay/conflict/concurrency, incident resolution, late exactly-once settlement, deterministic fault-hook configuration, TOTP time-step replay rejection, one-time backup-code consumption, persistent lockout across service instances, lockout recovery, disable evidence, OAuth provider/redirect/verifier binding, one-time state consumption, persisted consumed state, expiry rejection, and user-scoped usage query evidence |
...
| User Web | `npm run typecheck` and `npm run build` passed in `user-web` | Independent Solid client covers auth, OAuth callback, password recovery request/confirmation, dashboard, scoped usage, API keys, billing, profile, and TOTP setup/verify/disable; browser tests, email verification, and backup-code sign-in remain |
...
| PostgreSQL migrator | A fresh PostgreSQL volume applied one Orleans schema plus product migrations 001-023, including `022-auth-totp-state.sql` and `023-auth-oauth-states.sql`, then observed all twenty-four as `skip` on an explicit second run, exit 0 | No source database, CDC, compatibility table, snapshot, or old key used |
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `dd23bb4` and Gateway `9c7171f` in unique project `scalaapi-realtime-smoke-20260809`; the clean Gateway image was built with the immutable Photon pin and full FetchContent history. The stack applied all 22 migrations in that historical run, authenticated Garnet, settled/replayed Chat and realtime WebSocket, exercised restart/recovery and the Provider matrix, resolved one incident, and persisted/downloaded the MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, exactly-once recovery including dispatch retry and outbox claim reclaim, realtime usage settlement, safe never-forwarded expiry, late usage settlement, actual client cancellation retention, and nine unknown-charge incidents with eight remaining open after one decision. The current 23-migration baseline additionally has real PostgreSQL TOTP and OAuth PKCE state tests; hosted CI, runtime WebSocket soak, multi-instance recovery, and cross-protocol automation remain |
...
| User portal contract | User Web uses refresh-aware `/auth`, password recovery, scoped `/user/usage` and `/user/usage/balance`, API key, payment, subscription, profile, password, and TOTP contracts; repository test proves usage rows cannot cross user boundaries | Browser automation, email verification, backup-code sign-in, redeem/affiliate flows, and real payment provider checkout remain |
