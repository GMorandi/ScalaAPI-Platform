...
The latest source snapshot is Platform/Admin Web/User Web `c1bbb4d` and Gateway
`418da3a`.
...
public `data` item projection, Provider-backed image cancellation, and a hosted
orphan pass. Platform `1d7ec4f` signs paginated S3-compatible listing, compares
...
protected. Restart restore and full batch download/reconciliation remain separate
gates.
...
protects referenced and young objects, and deletes aged orphans. Restore/restart
race coverage, retention, and full MinIO lifecycle evidence remain open.
...
| Current Platform tests | 262/262, exit 0; Release build 0 warnings/0 errors | 69 Grain, 86 Host, 46 Admin, and 61 Provider tests; current additions cover signed S3 list pagination, media orphan protection/deletion, bounded batch archive ZIP creation and manifest/error entries, API-key-isolated durable image batch listing, and Provider image task/batch cancellation contracts, alongside three Embeddings provider profiles, catalog entries, dimensions, token accounting, and fault boundaries. The preceding coverage includes Responses compact path resolution, stable source-text input-item replay/deletion, idempotent cancellation, pricing precedence, Garnet revision publication, classifier metrics/secret-redaction, migrations 044-046, and backup schema coverage |
