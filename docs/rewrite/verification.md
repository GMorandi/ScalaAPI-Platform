...
`invalid_grant` profiles each entered a terminal secret-cleared `revoked` state,
advanced the credential generation, created no Provider request or financial
state, and recovered through an explicit replacement followed by one normal
settlement. Native Anthropic and Gemini clients then disconnected before output;
the Provider mock observed exactly one cancellation per request, while each path
persisted one `reconciliation_needed` lease, one active hold, forwarded plus
unknown-charge journal evidence, and no retry, output-start event, usage, log, or
debit. The existing OpenAI client-disconnect case passed the same conservative
financial boundary after Gateway normalized cancellation-woken chunk reads.

The same run applied and replay-skipped 51 migration records and passed native
credential/auth/rejection, 429/500/malformed/timeout/disconnect/wrong-media-type/
late-usage faults, Chat/Responses/Embeddings/realtime, Platform/Gateway replacement,
two-Silo object-storage/PostgreSQL partitions, Garnet, S3/media, accounting,
reconciliation, and audited operator resolution. It finished with 23 incidents:
one resolved by the operator path and 22 durably open unknown-charge cases. Release
build is zero-warning/zero-error; Platform tests pass 294/294, Gateway tests pass
127/127, both Web typecheck/build gates pass, Cap'n Proto digest/generated output
and retired-dependency scans pass, and the four Scheduler Dry benchmark children
all execute successfully. Final cleanup removed every current or stopped container
and all `scalaapi-credential-cancel-0811d` volumes/networks; only the three
long-lived `apitf_*` baseline volumes, `apitf_default`, and Podman's built-in
network remain.

The preceding complete source-built native stream-terminal project
...
replay-skipped all 50 migrations and passed the then-current matrix.
...
| Provider OAuth credential refresh | Platform `651a786`; `scalaapi-credential-cancel-0811d`, exit 0 | Refresh leasing, CAS completion, rotation, HTTPS/bounds, and secret-free failure remain green. A bounded `invalid_grant` now clears access/refresh/client-secret material, fences stale completion, advances the generation, blocks dispatch, and records `revoked`; explicit replacement advances again and recovers exactly once for Anthropic and Gemini. Add master-key rotation, multi-Silo refresh contention, live credentials, and production revocation drills |
...
| PostgreSQL migrator | Product migrations 001-050 plus the image-owned Orleans baseline (51 records); `scalaapi-credential-cancel-0811d` applied every record and the second migrator run skipped every record | Migration 050 extends the metadata-only refresh audit with a constrained terminal `revoked` outcome and monotonic generation transition. No source database, CDC, compatibility table, snapshot, old key, or compatibility mapping was used |
| Empty-volume Compose gate | Platform `651a786`, Gateway `04ec18c`, project `scalaapi-credential-cancel-0811d`, exit 0 | The full source-owned stack passed migrations/replay, Garnet, Chat/Responses/Embeddings/realtime, all three Provider protocols and fault matrices, OAuth terminal revoke/replacement, actual Provider-side native cancellation, Platform/Gateway replacement, S3/media transport and retention, one-Silo object-storage/PostgreSQL partitions, two-Silo repair, accounting, reconciliation, and audited operator resolution. Hosted CI, live adapters, long soak, and deployment security remain |
