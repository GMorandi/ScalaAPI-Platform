...
The latest source snapshot is Platform/Admin Web/User Web `feb8bf4` and Gateway
`f08e868`.

The latest source-built project `scalaapi-responses-cancel-0810a` exited zero.
In addition to the root Responses JSON/SSE and GET/cancel/DELETE control checks, it
posted `/v1/responses/{id}/cancel` twice and verified an idempotent `cancelled`
response plus GET persistence. PostgreSQL showed three aborted subresource leases
and three released holds, with zero usage events, usage logs, or NUMERIC usage
debits across the three controls. It also
...
| --- | --- | --- |
| OpenAI Responses runtime gate | `scalaapi-responses-cancel-0810a`, exit 0 | Platform `feb8bf4` and Gateway `f08e868` built from source on empty volumes. Provider mock Responses emits a terminal SSE event with usage; JSON and SSE requests returned valid envelopes and each settled one lease/event/log/committed hold/NUMERIC debit. GET `/v1/responses/{id}` replayed the stored response, POST `/v1/responses/{id}/cancel` returned the same `cancelled` response on replay and GET retained that state, and DELETE `/v1/responses/{id}` returned `response.deleted`; the three controls each released one non-billable lease with no usage/debit. Provider HTTP tests cover cancel replay and unknown IDs. A malformed non-stream 2xx fixture returned HTTP 502 `provider_error`, retained one reconciliation hold, and produced no usage/debit before the audited resolution pass. Remaining mutation semantics beyond read/cancel/delete, provider-group faults, and live adapters remain open |
