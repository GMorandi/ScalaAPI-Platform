...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web/User Web `6ae059b`.
...
| Cross-Gateway idempotency under restart | `scalaapi-multi-gateway-0810a`, exit 0 | The source-owned empty-volume smoke started two Platform silos and two Gateways, restarted the secondary pair, then sent concurrent normal Chat requests through both Gateways with one API-key idempotency key. Responses were limited to success or active replay; PostgreSQL proved one completed idempotency row, one lease, one usage event, and one NUMERIC `usage_debit`. The trap removed the isolated project and `podman ps -a` was empty |
