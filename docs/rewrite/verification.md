...
The latest source snapshot is Platform/Admin Web/User Web `1d7ec4f` and Gateway
`418da3a`.
...
| Current Platform tests | 260/260, exit 0; Release build 0 warnings/0 errors | 69 Grain, 85 Host, 46 Admin, and 61 Provider tests; current additions cover signed S3 list pagination, media orphan protection/deletion, API-key-isolated durable image batch listing, and Provider image task/batch cancellation contracts, alongside three Embeddings provider profiles, catalog entries, dimensions, token accounting, and fault boundaries. The preceding coverage includes Responses compact path resolution, stable source-text input-item replay/deletion, idempotent cancellation, pricing precedence, Garnet revision publication, classifier metrics/secret-redaction, migrations 044-046, and backup schema coverage |
