...
The latest source snapshot is Gateway `3da0d33` and Platform `326fc43`.
...
| Platform tests | 185/185, exit 0; Release build 0 warnings/0 errors | 69 Grain tests, 52 Host tests, 23 Admin tests, and 41 Provider mock tests; Host coverage includes versioned Unicode normalization, source-owned external classifier request/status/schema/timeout/cancellation behavior, redacted audit metadata, policy revision schema, successful/retryable propagation, concurrent two-worker claim serialization, staged audits, accounting, media, and reconciliation; Admin coverage adds atomic referral reward replay/conflict evidence, audited operational-metric summaries/alerts, bounded redacted audit export evidence, encrypted proxy/TLS profile validation, and bounded active-account monitor checks; Provider HTTP tests cover the classifier contract and deterministic fixtures |
