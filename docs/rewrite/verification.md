...
The latest source snapshot is Gateway `297b131` and Platform `bb7f2c0`.
...
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `bb7f2c0` and Gateway `297b131` source snapshots in unique project `scalaapi-platform-worker-recovery-0913`; the source-built stack applied all 22 migrations twice, authenticated Garnet, settled/replayed Chat, exercised Platform before-dispatch, after-outbox-claim, pre/post/pre-ack and Gateway before-dispatch/after-provider-completion crash recovery, replaced both services, ran the Provider matrix, resolved one incident, and persisted/downloaded the MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, ledger/projection drain, exactly-once recovery including outbox claim reclaim, safe never-forwarded expiry at both Platform and Gateway boundaries, late usage settlement, actual client cancellation retention, and nine unknown-charge incidents with eight remaining open after one decision. Hosted CI, final-usage replay/reconciliation, remaining Platform dispatch retry/Gateway hook matrix, and multi-instance evidence remain |
...
| Billable settlement smoke | JSON and SSE completed; SQL-authoritative hold committed; usage outbox processed; one versioned NUMERIC ledger debit per successful lease; account balance equalled ledger sum, max version equalled account version, versions were contiguous/distinct, and projection backlog reached 0. Platform before-provider-dispatch, after-outbox-claim, pre-settlement, post-settlement, and before-outbox-ack crashes plus Gateway before-provider-dispatch and after-provider-completion crashes were recovered by explicitly starting the same container; never-forwarded work expired safely at both dispatch boundaries, while forwarded ambiguity retained one reconciliation hold. Explicit non-stream and streaming 429/500 attempts released; malformed/disconnect/timeout-before-headers plus streaming disconnect, disconnect-before-output, malformed-usage, invalid-content-type, and downstream client-cancel attempts each retained one unknown hold. The truncated-stream profile settled valid usage exactly once before EOF; Admin settle replay returned `duplicate`; the latest gate ended with eight open incidents after nine were created | Remaining Platform dispatch retry details, other Gateway boundaries, and multi-instance recovery remain |
...
4. Extend the now-normalized `503/provider_unavailable` Provider reset contract to
   ...
   Exercise the remaining Platform dispatch-retry, other Gateway, and
