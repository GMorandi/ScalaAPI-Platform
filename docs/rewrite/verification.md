...
| --- | --- | --- |
| Gateway build and CTest | Clean local build; 92/92, exit 0 | Includes nested Anthropic start/final usage regression, TCP/TLS client, bounded response replay, malformed-usage/non-SSE guards, incomplete successful payload rejection, explicit-rejection evidence classification, terminal usage-outbox retirement, and Garnet invalidation flush recovery |
| Platform tests | 92/92, exit 0 | 57 Grain tests, 25 PostgreSQL-connected Host tests, 4 Admin tests, and 6 Provider mock selector tests; includes ordered accounting, replay/conflict, SQL holds, versioned projection, held/forwarded/output-started transitions, safe held expiry, unknown-abort hold preservation, incident resolution, and late exactly-once settlement |
...
| PostgreSQL migrator | Repository gate started a brand-new PostgreSQL volume, applied current image `8da18e9488a9` migrations 000-020, then observed all twenty-one as `skip` on an explicit second run, exit 0 | No source database, CDC, compatibility table, snapshot, or old key used |
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `6bfb974` and Gateway `84634d1` worktrees in unique project `scalaapi-smoke-evidence1`; source-built images were Platform `a8a9a83c8733`, Admin API `ee9803ff82eb`, Gateway `1cb2172da696`, Provider mock `4ccae2be4c03`, migrator `8da18e9488a9`, and Admin Web `19b0c422cc9d`. Product APIs created and funded a zero-balance user, configured key/groups/prices, settled/replayed Chat, replaced Platform and Gateway independently, separated explicit 429/500 rejections from three unknown-charge transport/protocol faults, authenticated Garnet, and downloaded the 67-byte MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, ledger/projection drain, and honest reconciliation failure with exactly three retained unknown-charge incidents. Hosted CI, normalized error contracts, client/SSE cancellation, exact-boundary crash injection, operator incident resolution, and multi-instance evidence remain |
...
| Request idempotency smoke | Concurrent same-key calls produced one 200 and one active-lease 409; after settlement a matching retry returned the original body; different fingerprint produced 409. Real-database coverage proves held expiry safely reopens the key, forwarded/unknown outcomes stay blocked in `reconciliation_needed`, evidence writes are idempotent, and late completion finalizes the original key once | Exact-boundary crash recovery and streaming replay semantics remain |
...
4. Add an audited idempotent operator resolution command for unknown charges.
   `release` requires explicit no-charge evidence; `settle` appends the normal
   versioned usage effect, and both actions retain actor/reason history. Normalize
   the direct-reset 502 versus post-cooldown 503 outcome into one public error
   contract, then extend the blocking non-stream matrix to SSE, partial output, and
   actual client disconnect. Inject process loss
