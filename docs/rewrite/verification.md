...
The latest source snapshot is Gateway `3da0d33`, Platform and User Web `ad6ac20`.
...
Payment-provider coupling, renewal workers, cross-Silo evidence, and browser
...
| Platform tests | 192/192, exit 0; Release build 0 warnings/0 errors | 69 Grain tests, 53 Host tests, 29 Admin tests, and 41 Provider mock tests; Host coverage includes versioned Unicode normalization, source-owned external classifier request/status/schema/timeout/cancellation behavior, redacted audit metadata, policy revision schema, successful/retryable propagation, concurrent two-worker claim serialization, staged audits, accounting, media, reconciliation, and serialized subscription quota reservation/settlement; Admin coverage adds atomic referral reward replay/conflict evidence, audited operational-metric summaries/alerts, bounded redacted audit export evidence, encrypted proxy/TLS profile validation, bounded active-account monitor checks, Passkey challenge/credential/counter lifecycle evidence, maintenance export redaction/cleanup/replay/conflict evidence, announcement read-state duplicate replay/audit evidence, and encrypted email outbox delivery/supersession/retry evidence; Provider HTTP tests cover the classifier contract and deterministic fixtures |
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-035 and skipped all 35 on replay; the migrator image copies the complete directory and includes migration 035 | The targeted empty-schema gate applies/skips 36 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `ad6ac20`; no source database, CDC, compatibility table, snapshot, or old key was used |
