...
The latest source snapshot is Gateway `3da0d33`, Platform `80ab783`, and User Web `45b75f8`.
...
| Platform tests | 187/187, exit 0; Release build 0 warnings/0 errors | 69 Grain tests, 52 Host tests, 25 Admin tests, and 41 Provider mock tests; Host coverage includes versioned Unicode normalization, source-owned external classifier request/status/schema/timeout/cancellation behavior, redacted audit metadata, policy revision schema, successful/retryable propagation, concurrent two-worker claim serialization, staged audits, accounting, media, and reconciliation; Admin coverage adds atomic referral reward replay/conflict evidence, audited operational-metric summaries/alerts, bounded redacted audit export evidence, encrypted proxy/TLS profile validation, bounded active-account monitor checks, Passkey challenge/credential/counter lifecycle evidence, and maintenance export redaction/cleanup/replay/conflict evidence; Provider HTTP tests cover the classifier contract and deterministic fixtures |
...
| User Web | `npm run typecheck` and `npm run build` passed in `user-web` at `45b75f8` | Independent Solid client covers auth, OAuth callback, password recovery, email verification, dashboard, scoped usage, API keys, active-plan subscription purchase/cancel/renew, redeem codes, referral summary, profile, TOTP setup/verify/disable, and Passkey registration/revocation/sign-in option conversion; browser tests, real authenticator ceremony, and backup-code sign-in remain |
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-032 and skipped all 32 on replay; the migrator image copies the complete directory and includes migration 032 | The targeted maintenance schema gate applies/skips 33 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `80ab783`; no source database, CDC, compatibility table, snapshot, or old key was used |
