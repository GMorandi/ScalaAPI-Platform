...
The latest source snapshot is Gateway `3da0d33` and Platform `ac560d5`.
...
| Passkey persistence and ceremony state | Platform `ac560d5`; temporary PostgreSQL, product migrations 001-031 applied then skipped, targeted Admin test 1/1 passed | `passkey_challenges` enforces flow-scoped five-minute one-shot challenges; `passkey_credentials` stores public material and monotonic signature counters; registration/revocation audit rows are transactionally paired. Fido2 registration/authentication endpoints issue normal sessions without private key storage. Browser WebAuthn ceremony, User Web controls, anti-enumeration, and distributed abuse limits remain open |
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-031 and skipped all 31 on replay; the migrator image copies the complete directory and includes migration 031 | The targeted Passkey schema gate applies/skips 32 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `ac560d5`; no source database, CDC, compatibility table, snapshot, or old key was used |
