...
| Ledger reconciliation probe | Current Admin image persisted run `id=1`, status `failed`, usage `0.00088560`, usage-debit ledger `0.48105060`, mismatch `-0.48016500`, and `missing_debit_count=2` | Existing long-lived smoke data must be isolated/repaired; a clean seeded run must pass before release |
