...
The latest source snapshot is Gateway `9c7171f` and Platform `c8dd39d`.
...
| Platform tests | 107/107, exit 0 | 59 Grain tests, 33 Host tests, 9 Admin tests, and 6 Provider mock selector tests; adds OAuth Provider credential lease serialization, atomic access/refresh rotation, metadata-update secret retention, refresh failure/backoff state, HTTPS/form token exchange, rotated-token parsing, and provider error-body redaction to the ordered accounting, identity, reconciliation, and user-scoping evidence |
...
| Provider OAuth credential refresh | Grain and Host tests pass for a single-account refresh lease, concurrent exclusion, CAS completion, rotated token/header hydration, safe metadata updates, persisted bounded failure/backoff, HTTPS-only token endpoint, bounded form response, and secret-free errors | Add Provider mock token endpoint, full dispatch/media runtime evidence, provider-specific parameters, audit history, and multi-Silo contention |
