...
The latest source snapshot is Gateway `3da0d33` and Platform `becf189`.
...
| Platform tests | 183/183, exit 0; Release build 0 warnings/0 errors | 69 Grain tests, 52 Host tests, 21 Admin tests, and 41 Provider mock tests; Host coverage includes versioned Unicode normalization, source-owned external classifier request/status/schema/timeout/cancellation behavior, redacted audit metadata, policy revision schema, successful/retryable propagation, concurrent two-worker claim serialization, staged audits, accounting, media, and reconciliation; Admin coverage adds atomic referral reward replay/conflict evidence, audited operational-metric summaries/alerts, and bounded redacted audit export evidence; Provider HTTP tests cover the classifier contract and deterministic fixtures |
