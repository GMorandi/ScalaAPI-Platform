...
The latest source snapshot is Platform/Admin Web/User Web `1cc4538` and Gateway
`418da3a`.
...
public `data` item projection, and Provider-backed image cancellation; orphan
cleanup, restart restore, and full batch download/reconciliation remain separate
gates.
...
listing/orphan cleanup, restore/restart race coverage, retention, and full MinIO
...
| Current Platform tests | 259/259, exit 0; Release build 0 warnings/0 errors | 69 Grain, 83 Host, 46 Admin, and 61 Provider tests; current additions cover API-key-isolated durable image batch listing, Provider image task/batch cancellation contracts, and the source-built batch list/items/cancellation gates, alongside three Embeddings provider profiles, catalog entries, dimensions, token accounting, and fault boundaries. The preceding coverage includes Responses compact path resolution, stable source-text input-item replay/deletion, idempotent cancellation, pricing precedence, Garnet revision publication, classifier metrics/secret-redaction, migrations 044-046, and backup schema coverage |
