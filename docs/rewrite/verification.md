...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web `911a22d`, and User Web `911a22d`.
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-040 and skipped all 40 on replay; the migrator image copies the complete directory and includes migration 040 | The targeted empty-schema gate applies/skips 41 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `911a22d`; no source database, CDC, compatibility table, snapshot, or old key was used |
