...
The latest source snapshot is Gateway `3da0d33`, Platform/Admin Web/User Web `ec502a2`.
...
workers plus a restarted worker with one sequence-one snapshot each. Actual
separate-process flush/restart remains a release gate.
...
| Current Platform tests | 239/239, exit 0; Release build 0 warnings/0 errors | 69 Grain, 77 Host, 45 Admin, and 48 Provider tests; Host includes the PostgreSQL-backed administrative-price precedence regression test, monotonic Garnet revision publication coverage, fixed-label OpenAI classifier metrics/secret-redaction assertions, and migrations 044-045 cross-instance snapshot/p95/error-ratio/budget-alert/window-recovery/hosted-worker replay coverage |
...
| User Web | Platform `ec502a2`; `npm run typecheck`, `npm run build`, and the local `npm run test:e2e` passed (`2/2`, live-only case skipped); source-built `scalaapi-public-ui-0810b` passed the live-only case (`1/1`) | Independent Solid client covers auth, OAuth callback, password recovery with action-link hydration, email verification with action-link hydration, dashboard announcements/read tracking, scoped usage, API keys, provider-selectable payment checkout links, active-plan subscription purchase/cancel/renew with reserved/remaining quota, redeem codes, referral summary, profile, TOTP setup/verify/disable, Passkey registration/revocation/sign-in option conversion, plus public model/status/terms/privacy routes. The live case proves the nginx-to-Gateway catalog/readiness proxies and legal routes from the built container; authenticated browser workflows, real authenticator ceremony, mail receipt, backup-code sign-in, deployment-specific accessibility scanning, and legal-text configuration remain |
...
| PostgreSQL migrator | The current baseline is product migrations 001-045 plus the image-owned Orleans baseline (46 records); a temporary empty PostgreSQL run applied and replay-skipped all 45 product migrations. Migration 043 widens only the greenfield classifier constraints to local/external/openai, while 044-045 add cross-instance classifier metric snapshots and durable budget-alert state | The current empty-volume bootstrap evidence is authoritative; no source database, CDC, compatibility table, snapshot, or old key was used |
