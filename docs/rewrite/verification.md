...
| Platform tests | 125/125, exit 0 | 59 Grain tests, 34 Host tests, 9 Admin tests, and 23 Provider mock tests; adds OAuth Provider credential lease serialization, atomic access/refresh rotation, metadata-update secret retention, refresh failure/backoff state, HTTPS/form token exchange, rotated-token parsing, provider error-body redaction, real HTTP Provider mock rotation/revocation/malformed/oversized response contracts, and PostgreSQL audit-history filtering to the ordered accounting, identity, reconciliation, and user-scoping evidence |
