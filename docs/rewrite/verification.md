...
The latest source snapshot is Gateway `1d03130` and Platform `713f5c0`.
...
The current source-built project `scalaapi-stream-policy-20260809b` exited zero.
It applied 29 empty-volume migration records (Orleans plus product migrations
001-028), skipped all 29 on the second migrator run, authenticated Gateway through
...
| Gateway build and CTest | Clean local build; 117/117, exit 0 | Adds revision-3 response-policy disposition, bounded event-boundary streaming policy buffering, OpenAI/Anthropic/Gemini policy-error SSE tests, explicit allow/block/fail-closed tests, and response error replay to the existing model catalog, Embeddings, nested Anthropic usage, SSE, timeout, cancellation, Garnet, retry, and routing coverage |
| Platform tests | 162/162, exit 0; Host 38/38 rerun against temporary PostgreSQL 17 | 69 Grain tests, 38 Host tests, 18 Admin tests, and 37 Provider mock tests; real-database Host coverage includes staged content-policy audits, rule-stage isolation/idempotency, fail-closed request-size limits, schema assertions, accounting, media, and reconciliation |
| Staged content policy | Gateway `1d03130`; Platform `6fc76f3` | Request rules block before lease creation; non-stream response rules run before delivery, audit the matched stage/rule, hide Provider output, preserve normal usage settlement, and replay the same HTTP 400. Streaming buffers complete bounded SSE events before client write and emits protocol-shaped terminal policy errors; the smoke withholds the first blocked event, retains one unknown-charge hold, and records no usage/debit. Unicode normalization, classifiers, alerts, and browser evidence remain open |
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-028 and skipped all 28 on replay; the migrator image now copies the complete directory and includes stage migration 028 | Compose installs Orleans support and the current empty-stack gate applies/skips 29 total records. No source database, CDC, compatibility table, snapshot, or old key was used |
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `713f5c0`/Gateway `1d03130` in unique project `scalaapi-stream-policy-20260809b`; the stack applied/skipped all 29 records, authenticated Garnet, settled/replayed Chat and realtime WebSocket, withheld the first blocked response SSE event while retaining its unknown-charge hold, exercised restart/recovery and the complete Provider matrix, resolved one incident, and persisted/downloaded the MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, exactly-once recovery including dispatch retry and outbox claim reclaim, request/response and streaming event-boundary policy behavior, safe never-forwarded expiry, late usage settlement, actual client cancellation retention, deterministic zero-output disconnect classification, and audited unknown-charge resolution. Hosted CI, runtime WebSocket soak, multi-instance recovery, classifier/Unicode fixtures, and cross-protocol automation remain |
