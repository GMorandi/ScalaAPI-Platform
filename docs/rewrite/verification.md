...
The latest source snapshot is Gateway `e7f6184` and Platform `713f5c0`.
...
The current source-built project `scalaapi-response-policy-20260809` exited zero.
...
| Gateway build and CTest | Clean local build; 114/114, exit 0 | Adds revision-3 response-policy disposition, explicit allow/block/fail-closed tests, and response error replay to the existing OpenAI Responses, model catalog, Embeddings, nested Anthropic usage, SSE, timeout, cancellation, Garnet, retry, and routing coverage |
...
| Staged content policy | Gateway `e7f6184`; Platform `6fc76f3` | Request rules block before lease creation; non-stream response rules run before delivery, audit the matched stage/rule, hide Provider output, preserve normal usage settlement, and replay the same HTTP 400. Streaming response moderation, classifiers, alerts, and browser evidence remain open |
...
| Empty-volume Compose gate | `deploy/stack/smoke.sh` passed from Platform `713f5c0`/Gateway `e7f6184` in unique project `scalaapi-response-policy-20260809`; the stack applied/skipped all 29 records, authenticated Garnet, settled/replayed Chat and realtime WebSocket, exercised restart/recovery and the complete Provider matrix, proved staged response-policy settlement/replay, resolved one incident, and persisted/downloaded the MinIO object | Source-owned gate proves ordered accounts, evidence-backed holds, exactly-once recovery including dispatch retry and outbox claim reclaim, request/response policy behavior, safe never-forwarded expiry, late usage settlement, actual client cancellation retention, deterministic zero-output disconnect classification, and audited unknown-charge resolution. Hosted CI, runtime WebSocket soak, multi-instance recovery, and cross-protocol automation remain |
