...
| Platform tests | 80/80, exit 0 | 55 grain tests, 22 PostgreSQL-connected Host tests, and 3 Admin webhook verifier tests; includes deterministic rolling quota precedence/reset, usage-triggered auth invalidation, response replay, immutable price snapshots, expiry recovery, restart-safe outbox claims, and idempotent balance-effect replay |
...
| PostgreSQL migrator | Current image contains 000-013; repeated invocation skipped all fourteen recorded migrations, exit 0 | No source database, CDC, or compatibility tables used; a truly empty-volume replay remains a release gate |
| Current-image Compose smoke | All long-running services healthy; migrator exit 0; Platform Silo force-recreated from `15bfff38`, current Gateway image `b072b760` returned readiness 200, and PostgreSQL showed zero pending/dead-lettered outbox rows and zero active holds; a seeded request stored `runtime-v1` rates and settled once | Isolated project and new volumes; full crash injection remains |
...
| Gateway dispatch smoke | Current Gateway image `b072b760` readiness 200; seeded OpenAI Chat JSON and SSE returned 200 through Provider mock, and a post-rebuild JSON request settled at `0.00006750` | Failure/retry matrix and clean-environment automation remain |
...
| Price snapshot smoke | Lease persisted `runtime-v1` and NUMERIC input/output rates; changing the in-memory price to `runtime-v2` before completion left the original cost unchanged; runtime request stored `2.50000000`/`10.00000000` | Authoritative Admin price-version lifecycle and historical backfill remain |
...
| Payment webhook state machine | Current Admin image created order `id=2`, accepted signed success once, replayed it as `duplicate=true`, accepted signed refund, and persisted `paid -> refunded` plus one `payment_credit` and one `payment_refund` ledger effect | Provider-specific adapters, reconciliation UI, and crash injection between SQL commit and balance projection remain |
