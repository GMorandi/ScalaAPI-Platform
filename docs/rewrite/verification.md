...
The latest source snapshot is Gateway `3da0d33` and Platform `f06cccc`.
...
| PostgreSQL migrator | A temporary empty PostgreSQL 17 database applied product migrations 001-031 and skipped all 31 on replay; the migrator image copies the complete directory and includes migration 031 | The targeted Passkey schema gate applies/skips 32 total records including Orleans support. The full Compose image gate must be rebuilt against Platform `f06cccc`; no source database, CDC, compatibility table, snapshot, or old key was used |
