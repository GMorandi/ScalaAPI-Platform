...
| Price snapshot smoke | Lease persisted `runtime-v1` and NUMERIC input/output rates; changing the in-memory price to `runtime-v2` before completion left the original cost unchanged; Admin published `stage2-price-1786199758`, rejected its duplicate, listed `2.75000000`/`11.25000000`, and closed it once | Live dispatch refresh from the authoritative table and historical backfill remain |
