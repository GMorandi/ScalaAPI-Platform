...
The latest source snapshot is Platform/Admin Web/User Web `f75fbfb` and Gateway
`418da3a`.
...
output-cleanup handlers. ScalaAPI now has the PostgreSQL-backed list route and
public `data` item projection; provider cancellation, orphan cleanup, restart
restore, and full batch download/reconciliation remain separate gates.
...
| Current Platform tests | 257/257, exit 0; Release build 0 warnings/0 errors | 69 Grain, 83 Host, 46 Admin, and 59 Provider tests; current additions cover API-key-isolated durable image batch listing and the source-built batch list/items gate, alongside three Embeddings provider profiles, catalog entries, dimensions, token accounting, and fault boundaries. The preceding coverage includes Responses compact path resolution, stable source-text input-item replay/deletion, idempotent cancellation, pricing precedence, Garnet revision publication, classifier metrics/secret-redaction, migrations 044-046, and backup schema coverage |
