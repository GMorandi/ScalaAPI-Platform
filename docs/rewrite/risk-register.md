...
| Pricing version or decimal precision error | P0 | Partial | Decimal-only business DTOs, fixed-scale Cap'n Proto fields, immutable NUMERIC lease snapshots, and Admin pricing publish/close validation pass; add live dispatch refresh, provider price adapters, and historical backfill tests |
