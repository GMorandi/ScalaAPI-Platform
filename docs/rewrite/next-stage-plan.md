...
The next stage starts from Platform `9491ea9`, Gateway `9c7171f`, and read-only
...
exposes password recovery and TOTP setup/verify/disable screens; browser evidence,
backup-code sign-in UX, email verification, and the remaining identity providers
are still open.
...
change, password recovery, and TOTP setup/verification/disable with backup-code
display. The API adds user-scoped usage and balance reads and the Compose stack
serves User Web separately from Admin Web. Browser automation, email verification,
backup-code sign-in UX, payment checkout, and redeem/affiliate workflows remain
open.
...
Progress in Gateway `9c7171f` and Platform `9491ea9`: the streaming pipe now requires a source protocol
...
3. Complete identity hardening beyond the TOTP and OAuth PKCE state machines, Passkeys,
   email verification and backup-code recovery UX, and browser tests for the new User Web/API-key/usage/order/
   subscription flows.
