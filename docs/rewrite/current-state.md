...
`scalaapi-responses-0810a` (Platform `c2f4e77`, Gateway `7b8b03c`). It includes
...
starve later durable usage reports; Gateway `7b8b03c` retains that event for
...
The same `scalaapi-responses-0810a` run exercised the OpenAI Responses root
endpoint through the source-owned Provider mock. Non-stream JSON validated the
completed response envelope, output text, and positive usage; SSE validated
`response.created`, `response.output_text.delta`, and terminal
`response.completed` events with nested usage. Both requests produced exactly
one lease, usage event, usage log, committed hold, and NUMERIC debit. Responses
subresources, provider-specific error fixtures, and live adapters remain open.

At Platform/Admin Web/User Web `c2f4e77` and Gateway `7b8b03c`:
