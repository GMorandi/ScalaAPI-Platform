...
`scalaapi-responses-subresource-0810b` (Platform `ac18ea9`, Gateway `7c774cb`). It includes
...
starve later durable usage reports; Gateway `7c774cb` retains that event for
...
The `scalaapi-responses-subresource-0810b` run exercised the OpenAI Responses
root endpoint and read subresource through the source-owned Provider mock.
...
Mutation subresources, provider-specific error fixtures, and live adapters
remain open.

At Platform/Admin Web/User Web `ac18ea9` and Gateway `7c774cb`:
