...
- Gateway: 52 production C++ source/header files, 11 test source files, and 125
  CTest cases.
- Platform: 120 hand-written production C# files, 5 generated Cap'n Proto/global C# files, 65 test/benchmark C# files, and 244 passing tests: 69 Grain, 81 Host, 46 Admin, and 48 Provider mock tests.
...
`scalaapi-responses-fault-0810e` (Platform `ef2ff8c`, Gateway `d9638b4`). It includes
...
and NUMERIC usage debits. The two control requests were explicitly no-charge:
...
starve later durable usage reports; Gateway `d9638b4` retains that event for
...
The `scalaapi-responses-fault-0810e` run exercised the OpenAI Responses root
endpoint and read/delete subresources through the source-owned Provider mock.
...
additional aborted routing lease with a released hold and no usage/debit.
...
mutation semantics, provider-group fault coverage, and live adapters remain
open.

At Platform/Admin Web/User Web `ef2ff8c` and Gateway `d9638b4`:
...
- Gateway built locally and passed 125/125 CTest cases, including deterministic
  ...
  status precedence.
- Platform Release test/build passed with 0 warnings and 0 errors: 244/244 tests,
  including 81 Host tests, 69 Grain tests, 46 Admin tests, and 48 Provider mock
