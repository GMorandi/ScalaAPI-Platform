...
public sealed class XaiQuotaClient(HttpClient http, ILogger<XaiQuotaClient> logger)
    : IProviderQuotaClient
{
    public string Platform => "xai";
    ...
    public async Task<ProviderQuotaInfo?> GetQuotaAsync(AccountCredentials credentials, CancellationToken ct)
    {
        try
        {
            ...
            {
                logger.LogWarning("xAI quota check failed with status {Status}", response.StatusCode);
                return null;
            }
        ...
        {
            logger.LogWarning(ex, "xAI quota check failed for account {AccountId}", credentials.Id);
            return null;
        }
    }
}
