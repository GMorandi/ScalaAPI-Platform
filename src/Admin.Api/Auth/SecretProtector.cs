...
public sealed class SecretProtector
{
    ...
    public string Unprotect(string protectedValue)
    {
        ...
        if (source.Length < 28) throw new CryptographicException("Invalid protected secret");
        var plaintext = new byte[source.Length - 28];
        using var aes = new AesGcm(_key, 16);
        aes.Decrypt(source.AsSpan(0, 12), source.AsSpan(28), source.AsSpan(12, 16), plaintext);
